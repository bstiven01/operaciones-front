...
export class CargueArchivoComponent implements OnInit {
  ...
  insertarProgramacionSemanal() {
    this._cargueService.validarArchivo(this.selectedFile).subscribe(
      (resp:Mensaje) => {
        this.spinner.hide();
        this.mensajeRespuesta = resp;
        if (this.mensajeRespuesta.codigoPeticion  === 0) {
          GeneralUtil.MENSAJE(resp.mensajePeticion, 'success');
        } else if (this.mensajeRespuesta.codigoPeticion  === 1 || this.mensajeRespuesta.codigoPeticion  === 4) {
  ...
  consultarCargue(tipo : string, tipoCargue : string){
    if(tipo === 'TODO'){
